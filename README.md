...
## Cloud Logic and Data Flow (Planned Implementation)

This section outlines the planned logic for how the solar panel cleaning system will operate in a real-world cloud-integrated environment. Although not fully implemented in the current version, the architecture below represents how the system will be deployed for live operations.

![Solar Panel](assets/architecture.png)
### Robot to Cloud Communication

- Each robot will be equipped with sensors and connectivity modules (Wi-Fi or LTE).
- After cleaning a panel, a robot will send data including:
  - `robot_id`, `panel_id`, `timestamp`, `status`, and `energy_left`.
- Data will be serialized in **JSON** format.

### Secure Transmission

- All communication between robots and cloud will use **HTTPS** or **MQTT**.
- Authentication will be handled using **JWT tokens** or **API keys**.
- This ensures data integrity, privacy, and device authentication.

### Cloud Storage

- Data will be validated and stored in cloud databases such as:
  - **Firebase Realtime Database**
  - **MongoDB Atlas**
- Each cleaning event will be saved as a structured record with metadata for traceability and analytics.

### Data Processing and Analytics

- Cloud-based functions (e.g., AWS Lambda, Google Cloud Functions) or stream processors like **Apache Spark** will:
  - Analyze robot performance
  - Detect frequently cleaned panels
  - Calculate energy efficiency and identify idle time
- These analytics help optimize performance over time.

### Dashboard Visualization

- A web-based dashboard (built with **React** or **Vue**) will allow users to:
  - Monitor cleaning schedules
  - View robot energy stats and locations
  - Track daily/weekly cleaning coverage

### Feedback Loop and Optimization

- Based on the analytics results, the system will:
  - Recommend optimal schedules
  - Reassign robots dynamically
  - Dispatch updates back to robots via the cloud

> **Note:**  
> This is the planned cloud architecture and logic to be implemented in the final version of the system. The current implementation is focused on scheduling logic and algorithm development.
